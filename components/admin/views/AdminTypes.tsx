...
export default function AdminTypes() {
  ...
  const [editing, setEditing] = useState<ACType | null>(null)
  ...
  const fetchRows = async () => {
    ...
    try {
      ...
      const res = await fetch(`/api/admin/types?${params.toString()}`)
  ...
  const openAdd = () => { setEditing(null); setName(''); setEditOpen(true) }
  const openEdit = (row: ACType) => { setEditing(row); setName(row.name); setEditOpen(true) }

  const saveRow = async () => {
    if (!name.trim()) return
    if (editing) {
      await fetch('/api/admin/types', { method: 'PATCH', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ id: editing.id, name: name.trim() }) })
    } else {
      const res = await fetch('/api/admin/types', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ name: name.trim() }) })
      if (!res.ok) {
        const j = await res.json().catch(()=>({}))
        if (res.status === 409) setExistsOpen(true)
        else alert(j?.error || 'Failed to add type')
        return
      }
    }
    setEditOpen(false)
  ...
  const doDelete = async () => {
    await fetch('/api/admin/types', { method: 'DELETE', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ ids: deletingIds }) })
    ...
    <div className="h-full overflow-y-auto">
      <div className="p-4 space-y-6 pb-6">
        ...
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 
          ...
          <h1 className="text-lg sm:text-2xl font-bold">Manage AC Types</h1>
      ...
      <Dialog open={editOpen} onOpenChange={setEditOpen}>
        <DialogContent>
          <DialogHeader>
            <DialogTitle>{editing? 'Edit AC Type' : 'Add AC Type'}</DialogTitle>
          </DialogHeader>
