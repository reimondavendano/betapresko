...
export default function AdminServices() {
  ...
  const doDelete = async () => {
    await fetch('/api/admin/services', { method: 'DELETE', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ ids: deletingIds }) })
  ...
  return (
    <div className="h-full overflow-y-auto">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 p-4 
        ...
        <h1 className="text-lg sm:text-2xl font-bold">Manage Services</h1>
        
        <div className="flex flex-col xs:flex-row gap-2 w-full sm:w-auto">
          <button
            className="flex items-center justify-center px-3 py-2 text-sm sm:text-base bg-red-500 
              rounded-lg hover:bg-red-600 transition-colors disabled:opacity-60"
          ...
          <button
            className="flex items-center justify-center px-3 py-2 text-sm sm:text-base bg-green-500 
