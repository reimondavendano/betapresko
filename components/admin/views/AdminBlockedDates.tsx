...
type Blocked = { id: string; name: string; reason: string | null; from_date: string; to_date: string; created_at: string }
...
export default function AdminBlockedDates() {
  ...
  const [editing, setEditing] = useState<Blocked | null>(null)
  ...
  const fetchRows = async () => {
    ...
    try {
      ...
      const res = await fetch(`/api/admin/blocked-dates?${params.toString()}`)
  ...
  const openAdd = () => { setEditing(null); setName(''); setReason(''); setFromDate(''); setToDate(''); setEditOpen(true) }
  const openEdit = (row: Blocked) => {
    setEditing(row)
    setName(row.name)
  ...
  const saveRow = async () => {
    if (!name.trim() || !fromDate || !toDate) return
    const from_date = fromDate
    const to_date = toDate
    if (editing) {
      const res = await fetch('/api/admin/blocked-dates', { method: 'PATCH', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ id: editing.id, name: name.trim(), reason: reason || null, from_date, to_date }) })
      if (!res.ok) {
      ...
      const res = await fetch('/api/admin/blocked-dates', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ name: name.trim(), reason: reason || null, from_date, to_date }) })
      if (!res.ok) {
        const j = await res.json().catch(()=>({}))
        if (res.status === 409) setExistsOpen(true)
  ...
  const doDelete = async () => {
    await fetch('/api/admin/blocked-dates', { method: 'DELETE', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ ids: deletingIds }) })
    ...
    <div className="h-full overflow-y-auto">
      <div className="p-4 space-y-6 pb-6">
        ...
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 
          ...
          <h1 className="text-lg sm:text-2xl font-bold">Manage Blocked Dates</h1>
            ...
            <tbody className="bg-white divide-y divide-gray-200">
              ...
              {rows.map(r => (
                <tr key={r.id}>
                  ...
                  <td className="px-6 py-4 whitespace-nowrap"><div className="text-sm text-gray-500">{r.from_date}</div></td>
                  <td className="px-6 py-4 whitespace-nowrap"><div className="text-sm text-gray-500">{r.to_date}</div></td>
                  <td className="px-6 py-4"><div className="text-sm text-gray-500 max-w-xl truncate" title={r.reason || ''}>{r.reason}</div></td>
      ...
      <Dialog open={editOpen} onOpenChange={setEditOpen}>
        <DialogContent className="max-w-lg">
          <DialogHeader>
            <DialogTitle>{editing? 'Edit Blocked Date' : 'Add Blocked Date'}</DialogTitle>
          </DialogHeader>
