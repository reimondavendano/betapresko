...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  try {
    ...
    if (req.method === 'POST') {
      ...
      // Duplicate check (case-insensitive exact)
      ...
      const { data, error } = await supabase
        .from('ac_types')
        .insert({ name: name.trim(), is_active: typeof is_active === 'boolean' ? is_active : true })
        .select('*')
        .single()
      if (error) return handleSupabaseError(error, res)
    ...
    if (req.method === 'PATCH') {
      const { id, name, is_active } = req.body as { id: string; name?: string; is_active?: boolean }
    ...
    if (req.method === 'DELETE') {
      ...
      const { error } = await supabase.from('ac_types').delete().in('id', ids)
      if (error) return handleSupabaseError(error, res)
