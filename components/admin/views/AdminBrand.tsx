...
export default function AdminBrand() {
  ...
  const openEdit = (row: Brand) => { setEditing(row); setName(row.name); setEditOpen(true) }

  const saveBrand = async () => {
    if (!name.trim()) return
    if (editing) {
      await fetch('/api/admin/brands', { method: 'PATCH', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ id: editing.id, name: name.trim() }) })
    } else {
      const res = await fetch('/api/admin/brands', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ name: name.trim() }) })
      if (!res.ok) {
        const j = await res.json().catch(()=>({}))
        if (res.status === 409) setExistsOpen(true)
        else alert(j?.error || 'Failed to add brand')
        return
      }
    }
    setEditOpen(false)
  ...
  const doDelete = async () => {
    await fetch('/api/admin/brands', { method: 'DELETE', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ ids: deletingIds }) })
        ...
        <DialogContent>
          ...
          <div className="space-y-2">
            ...
            <div className="flex justify-end gap-2 pt-2">
              ...
              <Button onClick={saveBrand}>{editing? 'Update' : 'Add'}</Button>
            </div>
          </div>
        </DialogContent>
      </Dialog>
